'''
Created on 04.07.2012

@author: gschoenb
'''

import logging
import subprocess

class DeviceTest(object):
    '''
    A generic class for a performance test.
    '''

    ## Base directory to write output files to
    baseDir = ''
    
    def __init__(self,testname,devicename):
        '''
        Constructor
        @param testname Name of the test, specifies the output file.
        @param devicename Name of the device to test.  
        '''
        
        ## The output file for the fio job test results.
        self.__testname = testname
        
        ## The data file for the test, can be a whole device also.
        self.__devicename = devicename
        
        ## The name of the figures generated by pyplot.
        self.__figures = []
        
        ## An overview of the measurement values to generate the plot from.
        self.__tables = []
        
    @staticmethod
    def setBaseDir(bdir):
        DeviceTest.baseDir = bdir
    
    def getTestname(self):
        ''' Return the name of the test, is the name of the output file also. '''
        return self.__testname
    
    def getDevName(self):
        ''' Return the name of the data file or device. '''
        return self.__devicename
    
    def getFigures(self):
        ''' Return the array of pyplot figures.'''
        return self.__figures
    
    def setFigures(self,fig):
        ''' Set the list of the pyplot figures.
        @param fig The figure list.
        ''' 
        self.__figures = fig
    
    def addFigure(self,fig):
        ''' Add a figure to the list.
        @param fig The figure to be added.
        '''
        self.__figures.append(fig)
    
    def addTable(self,tb):
        ''' Add an measurement value table overview.
        @param tb The table to add.'''
        self.__tables.append(tb)
    
    def getTables(self):
        ''' Return the measurement overview table.'''
        return self.__tables
    
    def getDevSizeKB(self):
        '''
        Get the device size in KByte.
        The function calls 'blockdev' to determine device sector size
        and sector count.
        @return [True,size] on success, [False,0] if an error occured.
        '''
        out = subprocess.Popen(['blockdev','--getss',self.__devicename],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        (stdout,stderr) = out.communicate()
        if stderr != '':
            logging.error("blockdev --getss encountered an error: " + stderr)
            return [False,0]
        else:
            sectorSize = int(stdout)
            out = subprocess.Popen(['blockdev','--getsz',self.__devicename],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
            (stdout,stderr) = out.communicate()
            if stderr != '':
                logging.error("blockdev --getsz encountered an error: " + stderr)
                return [False,0]
            else:
                sectorCount = long(stdout)
                if ((sectorCount * sectorSize) % 1024) != 0:
                        logging.error("blockdev sector count cannot be divided by 1024")
                        return [False,0]
                devSzKB = (sectorCount * sectorSize) / 1024
                logging.info("#Device" + self.__devicename + " sector count: " + str(sectorCount))
                logging.info("#Device" + self.__devicename + " sector size: " + str(sectorSize))
                logging.info("#Device" + self.__devicename + " size in KB: " + str(devSzKB))
                return [True,devSzKB]
            
    def getNumSecKB(self):
        '''
        Get the number of 1024 Byte sectors
        The function calls 'blockdev' to determine device sector size
        and sector count.
        @return [True,num] on success, [False,0] if an error occured.
        '''
        out = subprocess.Popen(['blockdev','--getss',self.__devicename],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        (stdout,stderr) = out.communicate()
        if stderr != '':
            logging.error("blockdev --getss encountered an error: " + stderr)
            return [False,0]
        else:
            sectorSize = int(stdout)
            out = subprocess.Popen(['blockdev','--getsz',self.__devicename],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
            (stdout,stderr) = out.communicate()
            if stderr != '':
                logging.error("blockdev --getsz encountered an error: " + stderr)
                return [False,0]
            else:
                sectorCount = long(stdout)
                if sectorSize == 512:
                    if (sectorCount % 2) != 0:
                        logging.error("blockdev sector count cannot be divided by 2")
                        return [False,0]
                    num = sectorCount / 2
                    logging.info("#Device" + self.__devicename + " sector count in 1024 Byte: " + str(num))
                    return [True,num]
                else:
                    logging.error("blockdev --getss returned block size not equal to 512")
                    return [False,0]
    
    def checkDevIsMounted(self):
        '''
        Check if the given device is mounted. As we work as
        super user it is slightly dangerous to overwrite
        a mounted partition.
        @return True if device is mounted, False if not.
        '''
        out = subprocess.Popen(['mount','-l'],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        (stdout,stderr) = out.communicate()
        if stderr != '':
            logging.error("mount -l encountered an error: " + stderr)
            exit(1)
        else:
            for line in stdout.split('\n'):
                if line.find(self.__devicename) > -1:
                    logging.info("#"+line)
                    return True
            return False
        
    def checkDevIsAvbl(self):
        '''
        Check if the given device is a valid partition.
        @return True if yes, False if not.
        '''
        out = subprocess.Popen(['cat','/proc/partitions'],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        (stdout,stderr) = out.communicate()
        if stderr != '':
            logging.error("cat /proc/partitions encountered an error: " + stderr)
            exit(1)
        else:
            for line in stdout.split('\n'):
                if line.find(self.__devicename[5:]) > -1:
                    logging.info("#"+line)
                    return True
            return False
        
    def makeSecureErase(self):
        '''
        Carries out a secure erase via hdparm for the given device.
        @return True if device is secure erased, False if not.
        '''
        frozen = True
        security = False
        logging.info("#Starting Secure Erase for device: "+self.__devicename)
        out = subprocess.Popen(['hdparm','-I',self.__devicename],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        (stdout,stderr) = out.communicate()
        if stderr != '':
            logging.error("hdparm -I encountered an error: " + stderr)
            return False
        else:
            for line in stdout.split('\n'):
                if line.find("frozen") > -1:
                    if line.find("not") > -1:
                        frozen = False
                        logging.info("#Not in frozen state")
            if frozen:
                logging.error("#Device still in frozen state")
                return False
            if not frozen:
                out = subprocess.Popen(['hdparm', '--user-master','u',
                                        '--security-set-pass','pwd',self.__devicename],
                                       stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                stdout,stderr = out.communicate()
                out.wait()
                if out.returncode != 0:
                    logging.error("#Error: command 'hdparm --user-master u --security-set-pass pwd returned an error code.")
                    logging.error(stderr)
                    return False
                else:
                    out = subprocess.Popen(['hdparm','-I',self.__devicename],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                    (stdout,stderr) = out.communicate()
                    if stderr != '':
                        logging.error("hdparm -I encountered an error: " + stderr)
                        exit(1)
                    else:
                        lines = stdout.split('\n')
                        for i,line in enumerate(lines):
                            if line.find("Master password") > -1:
                                if lines[i+2].find("not") == -1 and lines[i+2].find("enabled") > -1:
                                    security = True
                                    logging.info("#Successfully enabled security for hdparm")
                                    break
                                else:
                                    logging.info("#Security NOT enabled for hdparm")
                                    return False
                        if security:
                            out = subprocess.Popen(['hdparm', '--user-master','u',
                                                    '--security-erase','pwd',self.__devicename],
                                                   stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                            stdout,stderr = out.communicate()
                            out.wait()
                            if out.returncode != 0:
                                logging.error("#Error: command 'hdparm --user-master u --security-erase pwd returned an error code.")
                                logging.error(stderr)
                                return False
                            else:
                                logging.info("#Successfully carried out secure erase for "+self.__devicename)
                                #Check if security is diasbled again
                                out = subprocess.Popen(['hdparm','-I',self.__devicename],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                                (stdout,stderr) = out.communicate()
                                if stderr != '':
                                    logging.error("hdparm -I encountered an error: " + stderr)
                                    exit(1)
                                else:
                                    lines = stdout.split('\n')
                                    for i,line in enumerate(lines):
                                        if line.find("Master password") > -1:
                                            if lines[i+2].find("not") > -1 and lines[i+2].find("enabled") > -1:
                                                security = False
                                                logging.info("#Successfully deactivated security for hdparm.")
                                                return True
                                            else:
                                                #Try to disable security manually
                                                logging.info("#Security still enabled for hdparm, therefore calling disable.")
                                                out = subprocess.Popen(['hdparm', '--user-master','u',
                                                    '--security-disable','pwd',self.__devicename],
                                                   stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                                                stdout,stderr = out.communicate()
                                                out.wait()
                                                if out.returncode != 0:
                                                    logging.error("#Error: command 'hdparm --user-master u --security-disable pwd returned an error code.")
                                                    logging.error(stderr)
                                                    return False
                                                else:
                                                    logging.info("#Successfully deactivated security for hdparm.")
                                                    return True